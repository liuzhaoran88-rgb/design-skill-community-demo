window.weeklyUpdates = {
  "schema_version": "1.1",
  "generated_at": "2026-07-28T10:42:00.696Z",
  "source": {
    "repository": "JD-Design-Wiki/2C-DesignWiki",
    "branch": "main",
    "head": "e3225917368e3107e47186d5433b62bc4bdff19a"
  },
  "period": {
    "start": "2026-07-22",
    "end": "2026-07-28",
    "base_commit": "7296e2976bb64814830cef536ea55d3066f2cf2b"
  },
  "items": [
    {
      "id": "prototype-generation",
      "path": "jd-design-system-md-v16/product-architecture/local-life/Skill/prototype-generation/SKILL.md",
      "title": "原型生成",
      "scene": "设计交付",
      "summary": "将 PRD、低保真原型或需求说明转为可编辑的高保真 Relay 原型。",
      "author": "yanghanhui.1",
      "progress": "补充 PRD 前置分析、KV 头图、JD16 图标库与规范同步脚本。",
      "url": "http://jagile.jd.com/codingRoot/JD-Design-Wiki/2C-DesignWiki/tree/main/jd-design-system-md-v16%2Fproduct-architecture%2Flocal-life%2FSkill%2Fprototype-generation",
      "is_tool": false,
      "commit": "e3225917368e3107e47186d5433b62bc4bdff19a",
      "committed_at": "2026-07-28T18:01:20+08:00",
      "detail": {
        "badge": "本周已更新",
        "description": "将 PRD、低保真原型或需求说明转为可编辑的高保真 Relay 原型。",
        "contributors": [
          "yanghanhui.1",
          "xuzhongwei1"
        ],
        "preparation_title": "使用提示",
        "preparation": [
          "不默认产品原型合理 ，质疑结构 / 层级 / CTA。",
          "不直接照抄 ，要重组、优化、补全。",
          "先诊断再生成 ，没完成诊断与结构优化前不调 MCP。"
        ],
        "cases": [],
        "lifecycle_label": "2 条仓库记录",
        "lifecycle": [
          {
            "version": "e322591",
            "date": "2026-07-28",
            "name": "yanghanhui.1",
            "action": "prototype-generation 原型生成 Skill(新增 PRD分析/KV头图/图标库/同步脚本 + 42文件更新)。",
            "outcome": "本周更新"
          },
          {
            "version": "c00a610",
            "date": "2026-07-28",
            "name": "xuzhongwei1",
            "action": "新增骑士端文案检测插件 + 补充 Skill 目录(joyspace-docs/prototype-generation/voice-of-customer)。",
            "outcome": "仓库提交"
          }
        ],
        "source": {
          "type": "skill-md",
          "name": "SKILL.md",
          "path": "jd-design-system-md-v16/product-architecture/local-life/Skill/prototype-generation/SKILL.md",
          "version": "e322591",
          "updated": "2026-07-28",
          "size": "26KB",
          "url": "http://jagile.jd.com/codingRoot/JD-Design-Wiki/2C-DesignWiki/blob/main/jd-design-system-md-v16%2Fproduct-architecture%2Flocal-life%2FSkill%2Fprototype-generation%2FSKILL.md",
          "prompt": "用 prototype-generation skill，将 PRD、低保真原型或需求说明转为可编辑的高保真 Relay 原型。"
        },
        "category": "设计交付"
      }
    },
    {
      "id": "industry-market-analysis",
      "path": "jd-design-system-md-v16/horizontal/user-research/skills/industry-market-analysis/SKILL.md",
      "title": "行业市场分析",
      "scene": "研究与分析",
      "summary": "从行业趋势、竞品、频道体验和设计策略四个维度支撑垂类频道改版决策。",
      "author": "liuxi1269",
      "progress": "新增三档深度、七阶段执行流程与可离线汇报的 HTML 报告交付。",
      "url": "http://jagile.jd.com/codingRoot/JD-Design-Wiki/2C-DesignWiki/tree/main/jd-design-system-md-v16%2Fhorizontal%2Fuser-research%2Fskills%2Findustry-market-analysis",
      "is_tool": false,
      "commit": "f03ff640ce4bdcf64318ce8e03314ae72ce35111",
      "committed_at": "2026-07-23T20:29:42+08:00",
      "detail": {
        "badge": "本周已更新",
        "description": "从行业趋势、竞品、频道体验和设计策略四个维度支撑垂类频道改版决策。",
        "contributors": [
          "liuxi1269"
        ],
        "preparation_title": "使用提示",
        "preparation": [
          "必读 references/workflow-overview.md —— 10 维度 × 三档深度 × 7 阶段的一页纸全景。",
          "选档 references/foundations/三档深度对照表.md —— 轻/中/重,按场景选一档。",
          "建品类工作目录(如 ~/Desktop/宠物行业分析/ ),把频道现状截图、竞品截图放进去。"
        ],
        "cases": [],
        "lifecycle_label": "1 条仓库记录",
        "lifecycle": [
          {
            "version": "f03ff64",
            "date": "2026-07-23",
            "name": "liuxi1269",
            "action": "新增 industry-market-analysis 行业市场分析工作流。",
            "outcome": "本周更新"
          }
        ],
        "source": {
          "type": "skill-md",
          "name": "SKILL.md",
          "path": "jd-design-system-md-v16/horizontal/user-research/skills/industry-market-analysis/SKILL.md",
          "version": "f03ff64",
          "updated": "2026-07-23",
          "size": "8KB",
          "url": "http://jagile.jd.com/codingRoot/JD-Design-Wiki/2C-DesignWiki/blob/main/jd-design-system-md-v16%2Fhorizontal%2Fuser-research%2Fskills%2Findustry-market-analysis%2FSKILL.md",
          "prompt": "用 industry-market-analysis skill，从行业趋势、竞品、频道体验和设计策略四个维度支撑垂类频道改版决策。"
        },
        "category": "研究与分析"
      }
    },
    {
      "id": "prd-design-brief",
      "path": "jd-design-system-md-v16/product-architecture/local-life/Skill/prd-design-brief/SKILL.md",
      "title": "PRD 设计简报",
      "scene": "研究与分析",
      "summary": "读取 PRD 并定位受影响的场域、页面与组件，形成可追溯的设计 Brief。",
      "author": "xuzhongwei1",
      "progress": "新增迭代/创意判断、结构化模块清单和下游出稿确认门。",
      "url": "http://jagile.jd.com/codingRoot/JD-Design-Wiki/2C-DesignWiki/tree/main/jd-design-system-md-v16%2Fproduct-architecture%2Flocal-life%2FSkill%2Fprd-design-brief",
      "is_tool": false,
      "commit": "c00a6100491f021c743143cee39df75cf527b653",
      "committed_at": "2026-07-28T13:43:13+08:00",
      "detail": {
        "badge": "本周已更新",
        "description": "读取 PRD 并定位受影响的场域、页面与组件，形成可追溯的设计 Brief。",
        "contributors": [
          "xuzhongwei1"
        ],
        "preparation_title": "使用提示",
        "preparation": [
          "读取 PRD 并定位受影响的场域、页面与组件，形成可追溯的设计 Brief。"
        ],
        "cases": [],
        "lifecycle_label": "2 条仓库记录",
        "lifecycle": [
          {
            "version": "c00a610",
            "date": "2026-07-28",
            "name": "xuzhongwei1",
            "action": "新增骑士端文案检测插件 + 补充 Skill 目录(joyspace-docs/prototype-generation/voice-of-customer)。",
            "outcome": "本周更新"
          },
          {
            "version": "39f5874",
            "date": "2026-07-21",
            "name": "xuzhongwei1",
            "action": "新增 prd-design-brief PRD 分析 skill。",
            "outcome": "仓库提交"
          }
        ],
        "source": {
          "type": "skill-md",
          "name": "SKILL.md",
          "path": "jd-design-system-md-v16/product-architecture/local-life/Skill/prd-design-brief/SKILL.md",
          "version": "c00a610",
          "updated": "2026-07-28",
          "size": "12KB",
          "url": "http://jagile.jd.com/codingRoot/JD-Design-Wiki/2C-DesignWiki/blob/main/jd-design-system-md-v16%2Fproduct-architecture%2Flocal-life%2FSkill%2Fprd-design-brief%2FSKILL.md",
          "prompt": "用 prd-design-brief skill，读取 PRD 并定位受影响的场域、页面与组件，形成可追溯的设计 Brief。"
        },
        "category": "研究与分析"
      }
    },
    {
      "id": "design-cleanup",
      "path": "jd-design-system-md-v16/product-architecture/search-recommend-foundation/skills&tools/design-cleanup/SKILL.md",
      "title": "设计稿清洗",
      "scene": "设计交付",
      "summary": "为准备交付或入库的 Zero / Relay 设计稿清理隐藏、透明和空节点噪音。",
      "author": "zhenshaoan.kong",
      "progress": "新增保真副本、分界面清洗、失败证据保留与交付验收流程。",
      "url": "http://jagile.jd.com/codingRoot/JD-Design-Wiki/2C-DesignWiki/tree/main/jd-design-system-md-v16%2Fproduct-architecture%2Fsearch-recommend-foundation%2Fskills%26tools%2Fdesign-cleanup",
      "is_tool": true,
      "commit": "3bc3579ab888761ab0cd4a25fe7422b1684ea48e",
      "committed_at": "2026-07-23T10:48:02+08:00",
      "detail": {
        "badge": "本周已更新",
        "description": "为准备交付或入库的 Zero / Relay 设计稿清理隐藏、透明和空节点噪音。",
        "contributors": [
          "zhenshaoan.kong"
        ],
        "preparation_title": "使用提示",
        "preparation": [
          "为每个源 Frame 派生独立 runId ，依次完成本节后续流程。批次中的每个 Frame 都必须产生自己的可点击正式副本或待核验副本；单个 Frame 失败不隐藏、回滚或冒充其他 Frame 的结果。",
          "对当前 Frame 用生成器 preflight 只读获取源 Frame、所在 Page、完整节点数和截图。",
          "用生成器 clone 阶段直接调用一次 FrameNode.clone() ；不加载字体、不预读图片、不解析实例。副本放到同一 Page 现有内容右侧约 120px，执行期间命名为 清洗处理中｜runId｜原名称 。"
        ],
        "cases": [],
        "lifecycle_label": "1 条仓库记录",
        "lifecycle": [
          {
            "version": "3bc3579",
            "date": "2026-07-23",
            "name": "zhenshaoan.kong",
            "action": "引入 design-cleanup skill 交付形态。",
            "outcome": "本周更新"
          }
        ],
        "source": {
          "type": "skill-md",
          "name": "SKILL.md",
          "path": "jd-design-system-md-v16/product-architecture/search-recommend-foundation/skills&tools/design-cleanup/SKILL.md",
          "version": "3bc3579",
          "updated": "2026-07-23",
          "size": "19KB",
          "url": "http://jagile.jd.com/codingRoot/JD-Design-Wiki/2C-DesignWiki/blob/main/jd-design-system-md-v16%2Fproduct-architecture%2Fsearch-recommend-foundation%2Fskills%26tools%2Fdesign-cleanup%2FSKILL.md",
          "prompt": "用 design-cleanup skill，为准备交付或入库的 Zero / Relay 设计稿清理隐藏、透明和空节点噪音。"
        },
        "category": "设计交付"
      }
    },
    {
      "id": "instance-manage",
      "path": "jd-design-system-md-v16/product-architecture/huangliu-design/product-details/JD-ProductDetails-web/brain-frontend/public/design-kb/skills/instance-manage/SKILL.md",
      "title": "实例标签管理",
      "scene": "设计交付",
      "summary": "把待审核实例的新增、修改、删除与合并操作沉淀为可执行的标签管理流程。",
      "author": "wangyutong.72",
      "progress": "补充待审核标记、关键词引用、操作归档与场景数据同步。",
      "url": "http://jagile.jd.com/codingRoot/JD-Design-Wiki/2C-DesignWiki/tree/main/jd-design-system-md-v16%2Fproduct-architecture%2Fhuangliu-design%2Fproduct-details%2FJD-ProductDetails-web%2Fbrain-frontend%2Fpublic%2Fdesign-kb%2Fskills%2Finstance-manage",
      "is_tool": false,
      "commit": "7732191ec13235e9b3a9943856ea4d9ba13fad8a",
      "committed_at": "2026-07-23T10:44:00+08:00",
      "detail": {
        "badge": "本周已更新",
        "description": "把待审核实例的新增、修改、删除与合并操作沉淀为可执行的标签管理流程。",
        "contributors": [
          "wangyutong.72"
        ],
        "preparation_title": "使用提示",
        "preparation": [
          "add ：追加新实例到 instances.json 对应 area",
          "update ：修改已有实例的字段",
          "delete ：从 instances.json 移除实例"
        ],
        "cases": [
          {
            "title": "成功操作",
            "summary": "✓ add: beltCountdown（腰带倒计时）→ universalFloors.card1Area",
            "source": "SKILL.md · 执行报告",
            "result": "文档示例"
          },
          {
            "title": "跳过",
            "summary": "⚠️ update: nonExistentId — 未找到，已跳过",
            "source": "SKILL.md · 执行报告",
            "result": "文档示例"
          }
        ],
        "lifecycle_label": "3 条仓库记录",
        "lifecycle": [
          {
            "version": "7732191",
            "date": "2026-07-23",
            "name": "wangyutong.72",
            "action": "pending keyword_ref + UI 待审核标记 + 测试数据。",
            "outcome": "本周更新"
          },
          {
            "version": "7234827",
            "date": "2026-07-20",
            "name": "wangyutong.72",
            "action": "弹窗交互优化 + SKILL 替换范围限定。",
            "outcome": "仓库提交"
          },
          {
            "version": "56fb38e",
            "date": "2026-07-17",
            "name": "wangyutong.72",
            "action": "实例标签规范维护 MVP。",
            "outcome": "仓库提交"
          }
        ],
        "source": {
          "type": "skill-md",
          "name": "SKILL.md",
          "path": "jd-design-system-md-v16/product-architecture/huangliu-design/product-details/JD-ProductDetails-web/brain-frontend/public/design-kb/skills/instance-manage/SKILL.md",
          "version": "7732191",
          "updated": "2026-07-23",
          "size": "6KB",
          "url": "http://jagile.jd.com/codingRoot/JD-Design-Wiki/2C-DesignWiki/blob/main/jd-design-system-md-v16%2Fproduct-architecture%2Fhuangliu-design%2Fproduct-details%2FJD-ProductDetails-web%2Fbrain-frontend%2Fpublic%2Fdesign-kb%2Fskills%2Finstance-manage%2FSKILL.md",
          "prompt": "用 instance-manage skill，把待审核实例的新增、修改、删除与合并操作沉淀为可执行的标签管理流程。"
        },
        "category": "设计交付"
      }
    },
    {
      "id": "voice-of-customer",
      "path": "jd-design-system-md-v16/product-architecture/local-life/Skill/voice-of-customer/skill/SKILL.md",
      "title": "用户之声分析",
      "scene": "研究与分析",
      "summary": "将评价、工单和开放反馈整理为主题、趋势与可行动的问题线索。",
      "author": "xuzhongwei1",
      "progress": "补充通用反馈分析框架与骑士端场景接入。",
      "url": "http://jagile.jd.com/codingRoot/JD-Design-Wiki/2C-DesignWiki/tree/main/jd-design-system-md-v16%2Fproduct-architecture%2Flocal-life%2FSkill%2Fvoice-of-customer%2Fskill",
      "is_tool": false,
      "commit": "c00a6100491f021c743143cee39df75cf527b653",
      "committed_at": "2026-07-28T13:43:13+08:00",
      "detail": {
        "badge": "本周已更新",
        "description": "将评价、工单和开放反馈整理为主题、趋势与可行动的问题线索。",
        "contributors": [
          "xuzhongwei1"
        ],
        "preparation_title": "使用提示",
        "preparation": [
          "分析 App/产品的 用户反馈、用户原声、客诉、评价、问卷",
          "痛点挖掘 / 体验问题盘点 / 诉求分类 / 满意度归因",
          "把反馈数据做成 可视化看板、分析报告、用户之声页面"
        ],
        "cases": [],
        "lifecycle_label": "1 条仓库记录",
        "lifecycle": [
          {
            "version": "c00a610",
            "date": "2026-07-28",
            "name": "xuzhongwei1",
            "action": "新增骑士端文案检测插件 + 补充 Skill 目录(joyspace-docs/prototype-generation/voice-of-customer)。",
            "outcome": "本周更新"
          }
        ],
        "source": {
          "type": "skill-md",
          "name": "SKILL.md",
          "path": "jd-design-system-md-v16/product-architecture/local-life/Skill/voice-of-customer/skill/SKILL.md",
          "version": "c00a610",
          "updated": "2026-07-28",
          "size": "40KB",
          "url": "http://jagile.jd.com/codingRoot/JD-Design-Wiki/2C-DesignWiki/blob/main/jd-design-system-md-v16%2Fproduct-architecture%2Flocal-life%2FSkill%2Fvoice-of-customer%2Fskill%2FSKILL.md",
          "prompt": "用 voice-of-customer skill，将评价、工单和开放反馈整理为主题、趋势与可行动的问题线索。"
        },
        "category": "研究与分析"
      }
    }
  ]
};
